...
public class DatabaseManager {
	...
	public DatabaseManager() {
		...
		if(Main.getMySQL().isConnected()) {
			...
			mySQL.update("CREATE TABLE IF NOT EXISTS BungeeMute(`playerId` INT, `startMute` DATETIME, `endMute` DATETIME, `muteBy` INT, `muteReason` VARCHAR(256), `id` INT NOT NULL AUTO_INCREMENT, PRIMARY KEY (id))");
